...
TUNE_PORT_RANGE=""         # пусто — диапазон ядра не меняется
TUNE_CONNTRACK="auto"
TUNE_NIC="yes"
...
- **Системный резолвер не переключается, пока кеш не доказал работоспособность.**
  dnsmasq слушает `127.0.0.1:53`, а systemd-resolved — `127.0.0.53:53`, поэтому
  они сосуществуют. Модуль поднимает кеш, опрашивает его напрямую через `dig`
  и только после успешного ответа трогает `/etc/resolv.conf`. Если кеш не
  ответил или проверить его нечем — резолвер остаётся нетронутым.
  При откате симлинк на systemd-resolved восстанавливается именно как симлинк,
  а обычный файл — из резервной копии; публичные DNS не навязываются, чтобы не
  сломать узел у провайдера, который режет внешний резолвинг.
  `chattr +i` не применяется: immutable мешает восстановлению DNS.
  Неудача включения кеша возвращает код 1, а не молчаливый успех.
...
> `TUNE_PORT_RANGE` по умолчанию пуст — диапазон эфемерных портов не меняется.
> Если задаёте его, порты ваших сервисов (панель, диапазон port-hopping
> Hysteria2) должны лежать **вне** указанного диапазона, иначе ядро займёт их
> под исходящие соединения.
